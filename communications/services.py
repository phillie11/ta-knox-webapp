...
class OutlookEmailService:
    ...
    def _generate_tracking_token(self, invitation_id):
        ...
        try:
            ...
            return str(invitation_id)  # Fallback to plain ID
    ...
    def __init__(self):
        ...
        # KEEP YOUR WORKING SIGNATURE LOADING
    ...
    def _load_signature(self):
        """Load standardized company email signature"""
        return """
        <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #e0e0e0;">
            ...
            <!-- Company Details -->
            <div style="font-family: 'Helvetica', Arial, sans-serif; font-size: 10.5pt; color: #000000;">
                <p style="margin: 0; line-height: 1.4;">
                    <strong>TA Knox</strong><br>
                    TA Knox House<br>
                    Hollingworth Road<br>
                    Bredbury, Cheshire, SK6 2AR<br>
                    Tel 0161 430 3308 :: Email
                    <a href="mailto:enquiries@taknox.co.uk" style="color: #467886; text-decoration: underline;">enquiries@taknox.co.uk</a>
                    :: Web
                    <a href="http://www.taknox.co.uk" style="color: #467886; text-decoration: underline;">www.taknox.co.uk</a>
                </p>
            </div>

            <!-- Company Registration -->
            <div style="font-family: 'Calibri', Arial, sans-serif; font-size: 11pt; margin: 10px 0;">
                <p style="margin: 0;">
                    <strong>Company Reg No. <span style="color: #0078D4;">380 9060</span></strong>
                </p>
            </div>

            <!-- Legal Disclaimer -->
            <div style="font-family: 'Calibri', Arial, sans-serif; font-size: 11pt; color: #000000; margin-top: 15px;">
                <p style="margin: 0; line-height: 1.3; font-size: 10pt;">
                    <strong>This e-mail is confidential and intended solely for the use of the individual to whom it was addressed.
                    Any views or opinions presented are solely those of the author and do not necessarily represent those of T.A. Knox.
                    If you are not the intended recipient, be advised that you have received this e-mail in error and that any use,
                    dissemination, forwarding, printing or copying of this e-mail is strictly prohibited. Viruses: Although we have
                    taken steps to ensure that this e-mail and attachments are free from any virus, we advise that in keeping with
                    good computing practice the recipient should ensure they are actually virus free. If you received this e-mail in
                    error, please contact T.A. Knox on <span style="color: #0078D4;">0161 430 3308</span></strong>
                </p>
            </div>
        </div>
        """
    ...
    def send_email(self, to_email, subject, body, cc=None, attachments=None, reply_url=None, sharepoint_links=None, invitation_id=None, **kwargs):
        ...
        try:
            ...
            if ';' in to_email:
                recipients = [recipient.strip() for recipient in to_email.split(';') if recipient.strip()]
            elif ',' in to_email:
                recipients = [recipient.strip() for recipient in to_email.split(',') if recipient.strip()]
            else:
                recipients = [to_email.strip()] if to_email.strip() else []

            logger.info(f"📧 Original to_email: {to_email}")
            logger.info(f"📧 Split into {len(recipients)} recipients: {recipients}")

            if not recipients:
                logger.error(f"No valid recipients in: {to_email}")
                return False
            ...
            action_buttons = ""
            if invitation_id and kwargs.get('include buttons', True):
            ...
            email_message = {
                "message": {
                    ...
                    "toRecipients": [
                        ...
                        } for recipient in recipients  # ← USE recipients LIST
                    ],
                },
    ...
    # Replace the existing send_pending_tender_reminder method with this:

    def send_pending_response_reminder(self, invitation):
        """
        Dedicated method for sending response reminders to subcontractors who haven't responded
        Uses sc_deadline and includes response buttons
        """
        try:
            project = invitation.project
            subcontractor = invitation.subcontractor

            logger.info(f"📧 Sending pending response reminder to {subcontractor.email} for project {project.name}")

            # Use sc_deadline specifically for subcontractor deadlines
            if project.sc_deadline:
                deadline_str = project.sc_deadline.strftime('%d/%m/%Y at %H:%M')
                deadline_date = project.sc_deadline.strftime('%d/%m/%Y')
            else:
                deadline_str = "TBC"
                deadline_date = "TBC"

            subject = f"REMINDER: Tender Response Required - {project.name}"

            body = f"""
            <div style="margin-bottom: 20px;">
                <p>Dear {subcontractor.first_name or 'Sir/Madam'},</p>

                <p>We previously sent you a tender invitation for <strong>{project.name}</strong>.</p>

                <p>We haven't received your response yet. Please let us know if you will be tendering for this project.</p>

                <p>The tender deadline is <strong>{deadline_str}</strong>.</p>

                <p>Please respond as soon as possible to confirm your intention.</p>
            ...
            # Generate tracking token for response buttons
            tracking_token = self._generate_tracking_token(invitation.id)
            tracking_url = f"{settings.BASE_URL}{reverse('tenders:track_email', kwargs={'token': tracking_token})}"
            response_url = f"{settings.BASE_URL}{reverse('tenders:response', kwargs={'token': tracking_token})}"

            # Add response buttons for pending reminders
            body += f"""
            ...
            # Add tracking pixel (hidden)
            body += f'<img src="{tracking_url}" width="1" height="1" style="display:none;" />'

            # Send email WITHOUT SharePoint links but WITH response buttons
            result = self.send_email(
                to_email=subcontractor.email,
                subject=subject,
                body=body,
                sharepoint_links=[],  # NO SharePoint links in reminders
                invitation_id=invitation.id,
                include_buttons=False  # We've manually added buttons above
            )

            logger.info(f"📧 Pending response reminder send result: {result} for {subcontractor.email}")
            return result

        except Exception as e:
            logger.error(f"💥 Error sending pending response reminder: {str(e)}")
            logger.exception("Full traceback:")
            return False

    # Also add this method for backward compatibility:
    def send_enhanced_tender_reminder(self, invitation, reminder_type='pending'):
        """
        Wrapper method for backward compatibility
        Routes to the appropriate dedicated method based on reminder_type
        """
        try:
            if reminder_type == 'pending':
                return self.send_pending_response_reminder(invitation)
            elif reminder_type == 'will_tender':
                return self.send_will_tender_deadline_reminder(invitation)
            else:
                logger.error(f"Unknown reminder type: {reminder_type}")
                return False
        except Exception as e:
            logger.error(f"Error in send_enhanced_tender_reminder wrapper: {str(e)}")
            return False

    def send_tender_reminder(self, invitation):
        """
        UPDATED: Send tender reminder WITHOUT SharePoint links
        """
        try:
            project = invitation.project
            subcontractor = invitation.subcontractor
            ...
            # Build subject and deadline info
            subject = f"REMINDER: Tender Response Required - {project.name}"
            ...
            # Build email body WITHOUT SharePoint links
            body = f"""
            <div style="margin-bottom: 20px;">
                <p>Dear {subcontractor.first_name or 'Sir/Madam'},</p>

                <p>This is a reminder about the tender invitation for <strong>{project.name}</strong>.</p>

                <p>The tender deadline is <strong>{deadline_str}</strong>.</p>

                <p>Please respond to confirm your intention to tender for this project.</p>
            ...
            body += f"""
            <div style="margin: 30px 0; text-align: center;">
                <p>Please confirm your intention:</p>
                <div style="margin: 20px 0;">
                    <a href="{response_url}?response=accept" style="display: inline-block; padding: 12px 24px; background-color: #28a745; color: white; text-decoration: none; border-radius: 5px; margin: 0 10px;">
                        ✓ I Will Tender
                    </a>
            ...
            # Send email WITHOUT SharePoint links
            result = self.send_email(
                to_email=subcontractor.email,
                subject=subject,
                body=body,
    ...
    def send_will_tender_deadline_reminder(self, invitation):
        """
        Dedicated method for sending deadline reminders to subcontractors who confirmed they will tender
        Uses sc_deadline and specific messaging for tender submission reminders
        """
        try:
            project = invitation.project
            subcontractor = invitation.subcontractor

            logger.info(f"📧 Sending will tender deadline reminder to {subcontractor.email} for project {project.name}")
            ...
            subject = f"TENDER SUBMISSION DEADLINE REMINDER: {project.name}"
            ...
            <div style="margin-bottom: 20px;">
                ...
                <p>This is a reminder that the deadline for your tender submission is <strong>{deadline_str}</strong>.</p>

                <p>Please ensure your completed tender is submitted by this deadline.</p>

                <p>If you have any questions about the submission process or require clarification, please don't hesitate to contact us.</p>
            ...
            # Send email WITHOUT SharePoint links and WITHOUT response buttons
            result = self.send_email(
                to_email=subcontractor.email,
                subject=subject,
                body=body,
                sharepoint_links=[],  # NO SharePoint links in deadline reminders
                invitation_id=invitation.id,
                include_buttons=False  # NO buttons for will tender reminders
            )

            logger.info(f"📧 Will tender deadline reminder send result: {result} for {subcontractor.email}")
            return result

        except Exception as e:
            logger.error(f"💥 Error sending will tender deadline reminder: {str(e)}")
            logger.exception("Full traceback:")
            return False

    def send_tender_addendum(self, invitation, addendum):
        """
        FIXED: Send tender addendum - use form SharePoint link if provided
        """
        try:
            project = invitation.project
            subcontractor = invitation.subcontractor
            ...
            # Build email body using addendum description
            addendum_description = getattr(addendum, 'description', 'Please see the updated documents.')
            ...
            # CRITICAL: Check for FORM SharePoint link FIRST (same logic as invitations)
            sharepoint_links = []

            # 1. PRIORITY: Check if form SharePoint link was passed via addendum object
            if hasattr(addendum, '_form_sharepoint_link') and addendum._form_sharepoint_link:
                sharepoint_url = addendum._form_sharepoint_link
                link_title = getattr(addendum, '_form_sharepoint_description', 'Updated ITT Documents')
                logger.info(f"✅ Using SharePoint link from ADDENDUM FORM: {sharepoint_url}")
            ...
            # 2. FALLBACK: Use project SharePoint fields only if no form link provided
            elif hasattr(project, 'sharepoint_folder_url') and project.sharepoint_folder_url:
                sharepoint_url = project.sharepoint_folder_url
                link_title = getattr(project, 'sharepoint_link_description', 'Updated ITT Documents') or 'Updated ITT Documents'
                logger.info(f"📁 Fallback to project sharepoint_folder_url: {sharepoint_url}")
                ...
                link_title = getattr(project, 'sharepoint_link_description', 'Updated ITT Documents') or 'Updated ITT Documents'
                logger.info(f"📁 Fallback to project sharepoint_documents_link: {sharepoint_url}")
            ...
            if sharepoint_links:
                logger.info(f"📎 Using {len(sharepoint_links)} SharePoint links for addendum")
                for i, link in enumerate(sharepoint_links):
                    logger.info(f"   📎 Link {i+1}: {link['title']} -> {link['url']}")
            else:
                logger.warning(f"❌ NO SharePoint link provided for addendum to {project.name}")
            ...
            if not result:
                ...
                logger.warning(f"⚠️ Addendum sent but NO SharePoint links included for {subcontractor.email}")
            else:
                logger.info(f"✅ Addendum sent successfully with SharePoint links to {subcontractor.email}")
