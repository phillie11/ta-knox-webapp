...
class TenderInvitation(models.Model):
    ...
    last_pending_reminder = models.DateTimeField(null=True, blank=True, help_text="Last time pending reminder was sent")
    last_will_tender_reminder = models.DateTimeField(null=True, blank=True, help_text="Last time will tender reminder was sent")
    pending_reminder_count = models.IntegerField(default=0, help_text="Number of pending reminders sent")
    will_tender_reminder_count = models.IntegerField(default=0, help_text="Number of will tender reminders sent")
...
class RFIItem(models.Model):
    """Enhanced Request for Information items"""
    ...
    tender_analysis = models.ForeignKey('TenderAnalysis', on_delete=models.CASCADE, related_name='rfi_items', null=True, blank=True)

    # RFI Details
    rfi_number = models.CharField(max_length=20, blank=True, help_text="Auto-generated RFI number")
    category = models.CharField(max_length=20, choices=CATEGORY_CHOICES)
    priority = models.CharField(max_length=10, choices=PRIORITY_CHOICES, default='MEDIUM')

    # Question Details
    question = models.TextField(help_text="Clear, specific question requiring clarification")
    context = models.TextField(blank=True, help_text="Additional context for the question")
    document_reference = models.CharField(max_length=255, blank=True, help_text="Reference to source document")
    location_in_document = models.CharField(max_length=100, blank=True, help_text="Page/section reference")

    # Impact Assessment
    pricing_impact = models.CharField(max_length=20, choices=[
    ...
    risk_if_unresolved = models.TextField(blank=True, help_text="Risk if question remains unanswered")

    # Response Tracking
    status = models.CharField(max_length=15, choices=STATUS_CHOICES, default='PENDING')
    client_response = models.TextField(blank=True, help_text="Client's response to the RFI")
    internal_notes = models.TextField(blank=True, help_text="Internal notes about the RFI")
    ...
    def __str__(self):
        return f"{self.rfi_number} - {self.get_category_display()}"

    def save(self, *args, **kwargs):
        if not self.rfi_number and self.project:
            # Auto-generate RFI number
            existing_count = RFIItem.objects.filter(project=self.project).count()
            self.rfi_number = f"RFI-{self.project.reference or self.project.id}-{existing_count + 1:03d}"
        super().save(*args, **kwargs)

    @property
    def is_overdue(self):
        """Check if RFI response is overdue (more than 5 working days)"""
        if self.submitted_at and not self.responded_at:
            from datetime import timedelta
            return timezone.now() > self.submitted_at + timedelta(days=5)
        return False
